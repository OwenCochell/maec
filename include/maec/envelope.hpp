 ...
 *
 * In digital audio, an 'envelope' describes how a parameter of a sound changes
 * over time. For use in MAEC, you can think of an envelope as a value that
 * changes over time. Each envelope does not necessarily have to alter a
 * component of a sound, but most of the time they will be used to do so.
 *
 ...
 * implement these values, and will instead work with low-level envelopes such
 * as these.
 * TODO: Figure this out
 *
 * It is not recommended to use ANY envelopes for generating audio data!
 * Instead, these modules should be used to modulate the parameter of other
 * modules.
 *
 */
...
public:
     ...
     * has left ot return until it becomes invalid, i.e current time >= strop
     * time
     *
     * @return int
     */
    int64_t remaining_samples() {
        return (this->get_stop_time() - this->get_timer()->get_time()) /
               this->get_timer()->get_npf();
    }
};
 ...
 * to understand it's own duration, and the absolute time values should be
 * determined later.
 *
 ...
 * The idea is that the envelope to be managed should be configured with
 * necessary settings BEFORE it is managed within this envelope.
 *
...
public:
    ...
    void meta_process() override {
        this->env->meta_process();
        this->set_buffer(std::move(this->env->get_buffer()));
    }
};
 ...
 * Could be good to ensure speed for components that can guarantee their
 * structure
 */
...
public:
     ...
     * that being place certain envelopes in between two envelopes if there is
     * dead time. We use the "InternalEnvelope" for this, which simply returns
     * the stop value from the previous envelope.
     *
 ...
 *
 * - Attack is the time it takes to reach maximum value when key is initially
 * pressed
